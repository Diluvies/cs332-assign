...
trait Solver extends GameDef {
   ...
   */
  def done(b: Block): Boolean = { b.isStanding && b.b1 == goal }  // b1 == b2 if block is standing
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty)
      ...
      val newExplored = explored ++ newPath.map{ case (block, _) => block }

      // newly generated stream has always longer path than initial, so concat on back for ascending order sort
      initial #::: from(newPath, newExplored)
    }
  }
